		...
		tablesorterPager: new function() {
			...
			init = function(table, settings) {
				...
				.bind('pageSet pagerUpdate '.split(' ').join(namespace + ' '), function(e, num) {
                    ...
                    // Remove Rows With .remove-me (Added by Me kushii)
                    c.rowsCopy = c.rowsCopy.filter((row)=>{return !row.hasClass(c.selectorRemove.slice(1))})
                    if(p.totalRows!==c.rowsCopy.length){
                        p.totalRows = c.rowsCopy.length
                    }
                    if( c.rowsCopy.length%p.size === 0
                        && p.page+1 === p.totalPages
                        && c.rowsCopy !== 0 
                        && c.size !== 0 ){
                        moveToPrevPage(table , p)
                    }
                    //
					moveToPage(table, p, true);
					updatePageDisplay(table, p, false);
