			...
			return {
				send: function( headers, complete ) {
					...
					callback = function( _, isAbort ) {
						...
						if ( responses ) {
							var allResponseHeader;
							try{
								allResponseHeader = xhr.getAllResponseHeaders().trim().split('\r\n').reduce((acc, cur) => {
									const [x,v] = cur.split(': ');
									return Object.assign(acc, { [x] : isaN(v)? parseFloat(v) : v });
							  }, {})
							} catch(ex) {
								allResponseHeader = xhr.getAllResponseHeaders()
							}
							complete( status, 
								statusText, 
								responses, 
