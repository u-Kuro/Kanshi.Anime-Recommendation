self.onmessage = (message) => {
    ...
    if(!jsonIsEmpty(varScheme)){
        for(let i=0; i<animeEntries.length; i++){
            ...
            // Analyze
            var zformatMin = []
            if(varScheme[xformat]!==undefined) {
                zformatMin.push(varScheme[xformat])
            } else {
                zformatMin.push(varScheme.meanFormat-minNumber)
            }
            ...
            for(let j=0; j<xgenres.length; j++){
                ...
                if(varScheme[xgenres[j]]!==undefined) {
                    zgenresMin.push(varScheme[xgenres[j]])
                } else {
                    zgenresMin.push(varScheme.meanGenres-minNumber)
                }
                if(varScheme[xgenres[j]]!==undefined) {
                    if(varScheme[xgenres[j]]>=varScheme.meanGenres
                        &&genresIncluded[xgenres[j]]===undefined){
                        genresIncluded[xgenres[j]] = [xgenres[j].replace("Genre: ",""),varScheme[xgenres[j]]]
                    }
                }
            }
            var ztagsMin = []
            for(let j=0; j<xtags.length; j++){
                ...
                if(varScheme[xtags[j].name]!==undefined){
                    if(xtags[j].rank>=50 || varScheme[xtags[j].name]<varScheme.meanTags){
                        ztagsMin.push(varScheme[xtags[j].name])
                    } else {
                        ztagsMin.push(varScheme.meanTags-minNumber)
                    }
                }
                if(varScheme[xtags[j].name]!==undefined && xtags[j].rank>=50){
                    if(varScheme[xtags[j].name]>=varScheme.meanTags
                        &&tagsIncluded[xtags[j].name]===undefined){
                        tagsIncluded[xtags[j].name] = [xtags[j].name.replace("Tag: ",""),varScheme[xtags[j].name]]
                    }
                }
            }
            var zstudiosMin = []
            ...
            for(let j=0; j<xstudios.length; j++){
                ...
                if(varScheme[xstudios[j].name]!==undefined){
                    zstudiosMin.push(varScheme[xstudios[j].name])
                } else {
                    zstudiosMin.push(varScheme.meanStudios-minNumber)
                }
                if(varScheme[xstudios[j].name]!==undefined) {
                    if(varScheme[xstudios[j].name]>=varScheme.meanStudios
                        &&studiosIncluded[xstudios[j].name]===undefined){
                        studiosIncluded[xstudios[j].name] = [{[xstudios[j].name.replace("Studio: ","")]: xstudios[j].siteUrl},varScheme[xstudios[j].name]]
                    }
                }
            }
            var zstaffMin = {}
            ...
            for(let j=0; j<xstaff.length; j++){
                ...
                if(varScheme[xstaff[j].staff]!==undefined){
                    if(zstaffMin[xstaff[j].role]===undefined){
                        zstaffMin[xstaff[j].role] = [varScheme[xstaff[j].staff]]
                    } else {
                        zstaffMin[xstaff[j].role].push(varScheme[xstaff[j].staff])
                    }
                } else {
                    if(zstaffMin[xstaff[j].role]===undefined){
                        zstaffMin[xstaff[j].role] = [varScheme.meanStaff-minNumber]
                    } else {
                        zstaffMin[xstaff[j].role].push(varScheme.meanStaff-minNumber)
                    }
                }
                if(varScheme[xstaff[j].staff]!==undefined) {
                    if(varScheme[xstaff[j].staff]>=varScheme.meanStaff
                        &&staffIncluded[xstaff[j].staff]===undefined){
                        staffIncluded[xstaff[j].staff] = [{[xstaff[j].staff.replace("Staff",xstaff[j].role)]: xstaff[j].siteUrl},varScheme[xstaff[j].staff]]
                    }
                }
            }
            // Original Scores
