self.onmessage = (message) => {
    ...
    var animeEntries = Object.values(data.animeEntries??{})
    var savedUserScores = data.savedUserScores || {}
    if(!savedUserScores.all){
        savedUserScores.all = {}
    }
    if(!savedUserScores.above){
        savedUserScores.above = {}
    }
    ...
    if(!jsonIsEmpty(varScheme)){
        ...
        var savedUserAnimeIDs = Object.keys(savedUserScores.all)
        for(let i=0;i<savedUserAnimeIDs.length;i++){
            if(!userListStatus.userScore[savedUserAnimeIDs[i]]){
                delete savedUserScores.all[savedUserAnimeIDs[i]]
                delete savedUserScores.above[savedUserAnimeIDs[i]]
            }
        }
        var meanUserScore, meanScoreAll, meanScoreAbove, userScoreBase;
        ...
        for(let i=0; i<animeEntries.length; i++){
            ...
            var tags = anime?.tags || []
            var studios = anime?.studios?.nodes?.filter((studio)=> studio?.isAnimationStudio) || []
            ...
            if(typeof meanUserScore==="number"&&typeof userListStatus.userScore[anilistId]==="number"){
                if(typeof savedUserScores[anilistId]==="number"){
                    delete savedUserScores[anilistId]
                }
                savedUserScores.all[anilistId] = score
                if(userListStatus.userScore[anilistId]>=meanUserScore){
                    savedUserScores.above[anilistId] = score
                }
            }
        ...
        for(let i=0;i<savedRecSchemeEntries.length;i++){
            ...
            if(typeof meanScoreAll==="number"){
                savedRecScheme[savedRecSchemeEntries[i]].meanScoreAll = meanScoreAll
            } else {
                savedRecScheme[savedRecSchemeEntries[i]].meanScoreAll = 0
            }
            if(typeof meanScoreAbove==="number"){
                savedRecScheme[savedRecSchemeEntries[i]].meanScoreAbove = meanScoreAbove
            } else {
                savedRecScheme[savedRecSchemeEntries[i]].meanScoreAbove = 0
            }
            // Low Average
