...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Anime Recommendation Apps that uses existing Anilist account analyzed based through User. Kanshi.VI is based on User's Likeability and Variable Importance while Kanshi.R is based on other Anilist User Recommendations.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;d) To remove staffs shown in top similarities (Staffs will still be included in the algorithm), type [!top similarities: !staffs] or [!top similarities: staff] or [!similarities: staff] (Available Filters: staffs, studios, contents).  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;e) To only include genre and tags shown in top similarities (Genres and Tags will still be included in the algorithm), type [top similarities: contents] or [similarities: contents] (Available Filters: staffs, studios, contents).  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;f) To change the maximum variables shown in top similarities, type [limit top similarities: N] or [limit similarities: N] (Default Limit is 5).  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;g) To only allow Average Score of 60-100, type average score>=60.  
...
## Current Limitations
   - The Android app only runs on WebView so update pauses when app is not visible.
   - The Web page is static so updates may also stop when the tab is closed.
   - Anilist Rate Limits to 50 anime per page and 25 staff per page that takes that lot of request to finish before seeing some recommendation. To fix this individual updates for new anime, changes in anime status is included and transferred the long update at the end.
   - Kanshi.VI lags upon rendering huge list (10000+ anime). This can be temporarily fixed by adding filters i.e limit top wscore: N to limit visible anime recommendation.
